...
class ScriptManager(
    ...
    suspend fun scanScripts(): List<ScriptManifest> = withContext(Dispatchers.IO) {
    ...
    suspend fun getEngine(scriptId: String): ScriptEngine? {
        ...
        return engineTasks.getOrPut(scriptId) {
            scope.async {
                try {
    ...
    suspend fun importScript(uri: Uri): Result<Unit> = withContext(Dispatchers.IO) {
        try {
